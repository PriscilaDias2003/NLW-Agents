...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
    const response = await gemini.models.generateContent({
        ...
        contents: [
            {
                text: 'Transcreva o áudio para português do Brasil. Seja preciso e natural na transcrição. Mantenha a pontuação adequada e divida o texto em parágrafos quando for apropriedado.'
            },
            {
                inlineData: {
                    mimeType,
...
export async function generateAnswer(question: string, transcription: string[]) {
    ...
    const prompt = `
        Com base no texto fornecido abaxido como contexto, responda a pergunta de forma clara e precisa em português do Brasil.
